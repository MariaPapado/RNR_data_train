...
#                InpImages = torch.autograd.Variable(torch.from_numpy(Images), requires_grad=False).transpose(2,3).transpose(1, 2).type(torch.FloatTensor)
                InpImages = torch.autograd.Variable(Images, requires_grad=False).type(torch.FloatTensor)
                ROImap = torch.autograd.Variable(ROI.float(), requires_grad=False).unsqueeze(dim=1).type(torch.FloatTensor)
